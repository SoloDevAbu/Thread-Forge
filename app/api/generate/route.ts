  ...
  try {
    ...
    for (const platform of platforms) {
      ...
      try {
        ...
        // If no valid posts, create fallback posts
        if (validPosts.length === 0) {
          console.warn(`No valid posts generated for ${platform}, creating fallback`);
          const fallbackPosts = createFallbackPosts(content, platform, tone);
          ...
          // Use valid posts with sanitization
        ...
        // Create fallback posts on complete failure
        const fallbackPosts = createFallbackPosts(content, platform, tone);
        for (const post of fallbackPosts) {
          try {
            const { cleanContent, hashtags: contentHashtags } = sanitizePostContent(post.content);
            ...
            console.error('Failed to create fallback post:', dbError);
          }
        }
      }
    }
