...
export function HistorySidebar({ onSelectGeneration }: HistorySidebarProps) {
  ...
  return (
    <>
      <div
        className="fixed left-0 top-0 bottom-0 w-2 hover:w-4 bg-blue-500 opacity-0 hover:opacity-100 transition-all duration-200 ease-out cursor-pointer z-40"
        onMouseEnter={() => setIsOpen(true)}
        style={{ transition: 'width 200ms cubic-bezier(0.4, 0, 0.2, 1), opacity 200ms cubic-bezier(0.4, 0, 0.2, 1)' }}
      />
          ...
          <div className="p-6 border-b border-gray-200">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition bg-white text-gray-900 placeholder-gray-400"
              />
            </div>
          </div>
                    ...
                    {gen.platforms && gen.platforms.length > 0 && (
                      <div className="flex gap-1 mt-2">
                        {gen.platforms.map((platform) => (
                          <span
                            key={platform}
                            className="px-2 py-0.5 bg-white border border-gray-200 rounded text-xs capitalize text-gray-700"
                          >
                            {platform}
                          </span>
                        ))}
