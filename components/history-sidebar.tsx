...
export function HistorySidebar({ onSelectGeneration }: HistorySidebarProps) {
  ...
  return (
    <>
      <div
        className="fixed left-0 top-0 bottom-0 w-2 hover:w-4 bg-blue-500 dark:bg-blue-600 opacity-0 hover:opacity-100 transition-all cursor-pointer z-40"
        onMouseEnter={() => setIsOpen(true)}
      />

      <div
        className={`fixed left-0 top-0 bottom-0 w-96 bg-white dark:bg-[#0a0a0a] shadow-2xl dark:shadow-[0_0_50px_rgba(0,0,0,0.8)] transform transition-transform duration-300 z-50 ${
          ...
          <div className="p-6 border-b border-gray-200 dark:border-gray-900">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold text-gray-900 dark:text-gray-300">History</h2>
              </div>
              <button
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-900 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition bg-white dark:bg-[#050505] text-gray-900 dark:text-gray-400 placeholder-gray-400 dark:placeholder-gray-700"
              />
            </div>
          </div>
              ...
              <div className="p-4 space-y-3">
                {filteredGenerations.map((gen) => (
                  <button
                    ...
                    className="w-full text-left p-4 bg-gray-50 dark:bg-[#050505] hover:bg-blue-50 dark:hover:bg-gray-900 rounded-xl transition group border border-gray-200 dark:border-gray-900 hover:border-blue-300 dark:hover:border-blue-600"
                  >
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <p className="text-sm text-gray-900 dark:text-gray-400 line-clamp-2 font-medium">
                    ...
                    {gen.platforms && gen.platforms.length > 0 && (
                      <div className="flex gap-1 mt-2">
                        {gen.platforms.map((platform) => (
                          <span
                            key={platform}
                            className="px-2 py-0.5 bg-white dark:bg-black border border-gray-200 dark:border-gray-800 rounded text-xs capitalize text-gray-700 dark:text-gray-500"
                          >
                            {platform}
                          </span>
                        ))}
